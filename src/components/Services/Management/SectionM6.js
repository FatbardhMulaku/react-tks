...
const BackEnd_icon = ["BackEnd1","BackEnd2","BackEnd3","BackEnd4","BackEnd5","BackEnd6"];
...
function SectionM6() {
	...
	return (
		...
		>
		  ...
		  <Tab className={classes.tab} label="Traditional databases" {...a11yProps(5)} />
		  <Tab className={classes.tab} label="Big data" {...a11yProps(6)} />
		  <Tab className={classes.tab} label="DevOps" {...a11yProps(7)} />
		</Tabs>
		<TabPanel value={value} index={0} className={`${classes.TabPanel} BackEnd`}>
		  <h3>Back end</h3>
			...
			{BackEnd_icon.map((item, index) => {
				...
				<div className="row BackEnd_info">
					{t("Management.section_6.Back_end").map((item, index) => {
						return(
						<p key={index} className="col-6">{item.items}</p>
						)
					})}
